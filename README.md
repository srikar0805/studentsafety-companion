# 🛡️ Student Safety Companion (Campus Dispatch Copilot)

> **An AI-powered safety navigation system for university campuses.** Get safe walking routes with real-time crime data analysis, emergency phone locations, and context-aware safety tips.
...
## 🌐 Live Deployments
* **Frontend Application:** [Insert Frontend URL Here] 
...
cp .env.example .env
```
Add your `DATABASE_URL` and `ARCHIA_API_KEY` to `.env`.
...
### 5. Automated Data Scheduler (Background tasks)
```bash
# Keep crime data automatically updated
